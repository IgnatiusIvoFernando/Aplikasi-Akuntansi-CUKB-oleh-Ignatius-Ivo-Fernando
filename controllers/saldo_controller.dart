  ...
  Future<List<Map<String, dynamic>>> getSaldoAkhir() async {
    ...
    final result = await db.rawQuery('''
      SELECT 
        akun.id,
        akun.nama,
        k.nama as kategori_nama, // <--- Tambahkan kategori untuk analisis
        IFNULL(SUM(jd.debit), 0) AS total_debit, // Ubah nama kolom agar jelas
        IFNULL(SUM(jd.kredit), 0) AS total_kredit, // Ubah nama kolom agar jelas
        // [PENAMBAHAN KRUSIAL] Hitung Saldo Akhir
        (IFNULL(SUM(jd.debit), 0) - IFNULL(SUM(jd.kredit), 0)) AS saldo_akhir
      FROM akun
      JOIN kategori_akun k ON akun.kategori_id = k.id // <--- Tambahkan JOIN
      LEFT JOIN jurnal_detail jd ON jd.akun_id = akun.id
      GROUP BY akun.id
      ORDER BY akun.id // Lebih baik diurutkan berdasarkan ID akun
    ''');
    return result;
  }
